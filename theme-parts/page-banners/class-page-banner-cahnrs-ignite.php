...
class Page_Banner_CAHNRS_Ignite extends Theme_Part_Ignite {
	...
	protected function get_video_banner( $args, $context, $post_id ) {
		...
		if ( ! empty( $video_settings['height'] ) ) {

			$video_classes[] = $video_settings['height'];

		}

		if ( ! empty( $video_settings['content-position'] ) ) {

			$video_classes[] = $video_settings['content-position'];
